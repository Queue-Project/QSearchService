...
public class BranchUpdatedEventElasticSearchConsumerTests
{
    private readonly Mock<ILogger<BranchUpdatedEventElasticSearchConsumer>> _mockLogger;
    ...
    private readonly BranchUpdatedEventElasticSearchConsumer _consumer;

    public BranchUpdatedEventElasticSearchConsumerTests()
    {
        _mockLogger = new Mock<ILogger<BranchUpdatedEventElasticSearchConsumer>>();
        _mockContext = new Mock<ConsumeContext<BranchUpdatedEvent>>();
        _mockIndexService = new Mock<IElasticSearchIndexService>();
        _consumer = new BranchUpdatedEventElasticSearchConsumer(_mockLogger.Object, _mockIndexService.Object);
    }

    [Fact]
    public async Task Consume_Should_Update_Search_Document_For_Branch_When_Event_Received()
    {
        // Arrange
        ...
        // Assert
        _mockIndexService.Verify(x => x.UpdateAsync(
                It.IsAny<ElasticSearchIndex>(),
                It.IsAny<CancellationToken>()),
            Times.Once);
    }
