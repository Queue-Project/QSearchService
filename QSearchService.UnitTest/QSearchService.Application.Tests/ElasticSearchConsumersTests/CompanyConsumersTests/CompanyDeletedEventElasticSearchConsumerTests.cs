...
{
    public class CompanyDeletedEventElasticSearchConsumerTests
    {
        private readonly Mock<ILogger<CompanyDeletedEventElasticSearchConsumer>> _mockLogger;
        ...
        private readonly CompanyDeletedEventElasticSearchConsumer _consumer;

        public CompanyDeletedEventElasticSearchConsumerTests()
        {
            _mockLogger = new Mock<ILogger<CompanyDeletedEventElasticSearchConsumer>>();
            _mockContext = new Mock<ConsumeContext<CompanyDeletedEvent>>();
            _mockIndexService = new Mock<IElasticSearchIndexService>();
            _consumer = new CompanyDeletedEventElasticSearchConsumer(_mockLogger.Object, _mockIndexService.Object);
        }


        [Fact]
        public async Task Consume_Should_Delete_Search_Document_For_Company_When_Event_Received()
        {
            //Arrange
            ...
            _mockIndexService.Setup(s => s.DeleteAsync(expectedEvent.CompanyId, It.IsAny<CancellationToken>()))
            ...
            //Assert
            _mockIndexService.Verify(s => s.DeleteAsync(expectedEvent.CompanyId, It.IsAny<CancellationToken>()),
                Times.Once);
        }
    }
}